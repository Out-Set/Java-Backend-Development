...
import com.savan.quartz.scheduler.service.ITriggerListener;
import com.savan.quartz.scheduler.serviceimpl.SchedulerExecutionStateData;
...
public class TriggerListener implements ITriggerListener {
	...
	public void triggerFired(Trigger trigger, JobExecutionContext context) {
		...
		try {
			List<JOBSchedulerDetail> theJOBSchedulerDetails = jobSchedulerDetailService.findByProcessGroupHeaderIdAndJobName(Long.valueOf(trigger.getJobKey().getGroup()), trigger.getJobKey().getName());
			...
			if (ValidatorUtils.isNull(schedulerJobExecutionMonitorVO)) {
				schedulerJobExecutionMonitorVO = new SchedulerJobExecutionMonitorVO.SchedulerJobExecutionMonitorVOBuilder().setJobGroupName(trigger.getJobKey().getGroup())
	...
	private Boolean isAnyInstanceOfGivenSchedulerIsInProgress(SchedulerJobExecutionMonitorVO schedulerJobExecutionMonitorTempVO, Character schedulerStatusEnum) {
		return ValidatorUtils.notNull(schedulerJobExecutionMonitorTempVO)
				...
				&& quartzJOBSchedulerService.isJobSchedulerThreadAlive(schedulerJobExecutionMonitorTempVO.getThreadName())
